...
public class ListBoostTask extends AsyncTask<Void, TaskInfo, List<TaskInfo>> {
    ...
    public ListBoostTask(OnTaskListListener onTaskListListener) {
        mContext = PhoneCleanerApp.getInstance();
        ...
        if (Build.VERSION.SDK_INT <= 21) {
            ...
            do {
                ...
                try {
                    ...
                    if (!packagename.contains(mContext.getPackageName()) && Toolbox.isUserApp(applicationInfo)) {
                        if (runproInfo.importance == 130
                            ...
                            info.setChceked(!Toolbox.checkLockedItem(mContext, packagename));
                            info.getAppInfo();
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                for (ActivityManager.RunningServiceInfo runningServiceInfo : am.getRunningServices(Integer.MAX_VALUE)) {
                    try {
                        ...
                        if (!packageInfo.packageName.contains(mContext.getPackageName()) && Toolbox.isUserApp(applicationInfo)) {
                            TaskInfo info = new TaskInfo(mContext, applicationInfo);
                            info.setChceked(!Toolbox.checkLockedItem(mContext, mContext.getPackageName()));
